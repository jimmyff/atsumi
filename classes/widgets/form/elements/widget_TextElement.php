...
class widget_TextElement extends widget_AbstractElement {
	...
	public function __construct($args) {
        ...
        if (array_key_exists('onKeyup', $args) && strlen($args['onKeyup']))
            $this->onKeyup = $args['onKeyup'];

        if (array_key_exists('onKeypress', $args) && strlen($args['onKeypress']))
	...
	function renderElement() {
		...
		return(sf('<input type="%s" name="%s" value="%s" %s id="form_%s" class="%s" %s%s%s%s%s%s%s%s%s />',
			$this->htmlType,
			$this->getName(),
            ...
            !is_null($this->onKeyup) && strlen($this->onKeyup)?sf(' onKeyup="%s"', $this->onKeyup):'',
            !is_null($this->onKeydown) && strlen($this->onKeydown)?sf(' onKeydown="%s"', $this->onKeydown):'',
            !is_null($this->onKeypress) && strlen($this->onKeypress)?sf(' onKeypress="%s"', $this->onKeypress):'',
